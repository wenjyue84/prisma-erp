...
## Workspace sync (matching prismaerp.click)

The `lhdn_payroll_integration` app's `after_migrate` hook (`install.py`) automatically creates workspaces that match the cloud but aren't provided by any upstream app:

| Workspace name | Display label | Module | Why needed |
|----------------|--------------|--------|------------|
| Malaysia Compliance | E-Invoice | Myinvois Erpgulf | myinvois_erpgulf has duplicate `fixtures=` in hooks.py — second assignment overwrites first, losing workspace fixture |
| HR | HR | HR | hrms app doesn't define a top-level "HR" workspace |
| ERP Settings | ERP Settings | Setup | Custom workspace (not in any app's fixture) |
| ESS Mobile | ESS Mobile | Employee Self Service | Custom workspace |
| LHDN Payroll | LHDN Payroll | LHDN Payroll Integration | Provided by lhdn_payroll_integration fixture |

The hook also fixes the Module Def name: if a DB restore renamed "Myinvois Erpgulf" to "E-Invoice", the hook renames it back (required because `get_module_app()` uses `modules.txt`, not the DB).

**Manual sync (if needed):**

```bash
docker cp sync_cloud_workspaces.py prisma-erp-backend-1:/home/frappe/frappe-bench/apps/frappe/frappe/sync_cloud_workspaces.py
docker exec prisma-erp-backend-1 bench --site frontend execute frappe.sync_cloud_workspaces.fix_module
docker exec prisma-erp-backend-1 bench --site frontend execute frappe.sync_cloud_workspaces.create_workspaces
```
