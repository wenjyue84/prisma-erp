...
@frappe.whitelist(allow_guest=True, methods=["POST"])
def demo_login():
	"""Auto-login as Administrator for demo/testing. Ensures demo data is present."""
	frappe.local.login_manager.login_as("Administrator")

	# Ensure demo data exists (idempotent — skips if already present)
	if not frappe.db.exists("Company", "Arising Packaging"):
		try:
			from frappe import setup_test_data
			setup_test_data.run()
			frappe.db.commit()
		except Exception:
			frappe.log_error("Demo data setup failed", "Demo Login")
