...
# Directories to include in filesTouch (relative to repo root)
INCLUDE_DIRS = {"lhdn_payroll_integration/", "prisma_assistant/", "tests/"}
...
def extract_keywords(title: str) -> set[str]:
    ...
    # Remove US-NNN prefix
    title = re.sub(r"^US-\d+\s*[-:–—]\s*", "", title)
...
def get_completed_story_files(repo_root: str) -> dict[str, list[str]]:
    """
    Scan git log for 'feat: US-NNN' commits and extract files changed.
    Returns: {story_id: [file_paths]}
    """
    story_files: dict[str, list[str]] = {}

    try:
        # Get all commits matching feat: US-* pattern
        result = subprocess.run(
            ["git", "-C", repo_root, "log", "--oneline", "--all",
             "--grep=feat: US-", "--format=%H %s"],
        ...
        for line in result.stdout.strip().split("\n"):
            ...
            # Extract US-NNN from message
            match = re.search(r"US-(\d+)", message)
            if not match:
                continue
