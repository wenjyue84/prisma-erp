...
frappe.provide("lhdn_payroll_integration.chat");

$(document).on("app_ready", function () {
	lhdn_payroll_integration.chat.widget = new LHDNChatWidget();
	lhdn_payroll_integration.chat.widget.init();
});

class LHDNChatWidget {
	init() {
		this.history = [];
		this.is_open = false;
	...
	send() {
		...
		frappe.call({
			method: "lhdn_payroll_integration.api.chat.send_message",
			args: {
				message: text,
