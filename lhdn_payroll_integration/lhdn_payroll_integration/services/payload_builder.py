...
# Employer statutory contributions — NOT payments to employee-as-supplier.
# These are the employer's own liability to government agencies and must
# be excluded from self-billed invoice lines.
EMPLOYER_STATUTORY_COMPONENTS = frozenset({
    'EPF - Employer', 'SOCSO - Employer', 'EIS - Employer',
    'HRDF', 'HRDF Levy',
    'Employer EPF', 'Employer SOCSO', 'Employer EIS',
...
def build_salary_slip_xml(docname):
    ...
    # Filter out employer statutory contributions and custom-excluded components
    eligible_earnings = [
    ...
    # Calculate totals from eligible earnings ONLY (never use YTD fields)
    total_excl = sum(_quantize(Decimal(str(e.amount)) * conv_rate) for e in eligible_earnings)
    ...
    # InvoiceLines (one per eligible earnings row)
    for idx, earning in enumerate(eligible_earnings, start=1):
